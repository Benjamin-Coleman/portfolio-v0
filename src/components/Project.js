  ...
  componentDidMount() {
    ...
    this.props.projects.forEach((project, i) => {
      if (
        ...
        this.props.match.params.title.toLowerCase()
      ) {
  ...
  goPrevProject = () => {
    ...
    const tl = new TimelineMax()
      .to(splashScreen, 1, {
        opacity: 1,
        ease: "Power2",
        onComplete: readyNext
      })
      .to(splashScreen, 1, {
        opacity: 0,
        ease: "Power2",
        onComplete: () => splashScreen.classList.add("--is-hidden")
      });
  };
  ...
  render() {
    const projectInfo = this.props.currentProject.data.map((section, i) => (
      <section className="project-section" key={i}>
        <p className="project-section-text">{section.description}</p>
        <ReactPlayer
          key={i}
    ...
    const prevProject =
      this.props.currentIndex > 0
        ? this.props.projects[this.props.currentIndex - 1]
        : this.props.projects[this.props.projects.length - 1];
    const nextProject =
      this.props.currentIndex >= this.props.projects.length - 1
    ...
    return (
      <div>
        <DelayLink
          to="/"
          delay={1000}
          onDelayStart={this.closeProjectAnimation}
        >
        ...
        <div className="project-info-wrapper">
          ...
          <div className="project-overview-wrapper">
            ...
            {this.props.currentProject.link.url !== "" ? (
              <a href={this.props.currentProject.link.url} target="blank">
                {this.props.currentProject.link.label}
              </a>
            ) : null}
            {this.props.currentProject.link.github !== "" ? (
              <a href={this.props.currentProject.link.github} target="blank">
                Github
              </a>
            ) : null}
        ...
        <div className="project-story-wrapper">{projectInfo}</div>
        <div className="project-story-end">
          <Link to={`/projects/${prevProject.symbol}`}>
            <div
              className="project-prev"
              style={{ backgroundImage: `url(${prevProject.img.url})` }}
              onClick={this.goPrevProject}
            >
          ...
          <Link to={`/projects/${nextProject.symbol}`}>
            <div
              ...
              style={{ backgroundImage: `url(${nextProject.img.url})` }}
              onClick={this.goNextProject}
            >
              <h3>Next Project</h3>
            </div>
          </Link>
...
const mapStateToProps = state => ({
  ...
  currentProject:
    state.projectReducer.projects[state.projectReducer.currentIndex]
});
